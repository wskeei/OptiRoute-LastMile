...
- **单一可编辑主配送站**：设置页、调度页、监控地图、包裹录入、快递员录入和演示数据生成都会读取同一个当前主站点。
- **系统管理操作**：支持围绕当前站点重置样本、随机切换到全国其他支持城市后再生成样本、清空调度历史，以及查看包裹和快递员数据。
...
调度流程仍然保持现有的 Haversine 直线距离 + 约束 K-Means + 遗传算法组合，没有恢复真实路网或多站点调度选择。设置页现在用于维护唯一主配送站，以及执行站点感知的样本重置动作。
...
3. 打开 `http://localhost:5173`，登录后：
   - 可先进入 `设置` 编辑主配送站
   - 进入 `AI调度中心`
   - 点击 `重置数据`，或在 `设置` 中选择按当前站点重置 / 随机城市重置
   - 点击 `开始AI调度`
   - 观察后台优化过程中地图路线的实时更新
